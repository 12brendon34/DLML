...
bool Add_Source_Hook(char const* Path, int FFSAddSourceFlags) {
	return Add_Source(Path, FFSAddSourceFlags);
}
...
BOOL CreateHooks(HMODULE hmodule) {
	...
	if (MH_CreateHook(LogSettingsInstance_Address, &LogSettingsInstance_Hook, reinterpret_cast<void**>(&LogSettingsInstance)) != MH_OK)
	{
		std::string errorMessage = std::string("Unable to create LogSettingsInstance hook at location: %p"), LogSettingsInstance_Address;
		Log(errorMessage);
		throw std::runtime_error(errorMessage);
	}
